...
const BookMark = ({ status, ...rest }) => {
  ...
  const getClassNameToButton = () => {
    return status ? "btn btn-success" : "btn btn-outline-success";
  };

  return (
    <button {...rest} className={getClassNameToButton()}>
      {onBookmarkChange()}
      {/*<i className={"bi bi-box2" + (status ? "-heart" : "")}></i>*/}
    </button>
  );
};
